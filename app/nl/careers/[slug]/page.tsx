...
const T = translations.nl.careers;
...
export async function generateStaticParams() {
  return articles.filter((a) => a.lang === "nl").map((a) => ({ slug: a.slug }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const url = `https://cvixeo.com/nl/careers/${article.slug}`;
...
export default async function ArticlePageNl({ params }: Props) {
  const { slug } = await params;
  const article = getArticleBySlug(slug);

  if (!article || article.lang !== "nl") {
    notFound();
  }
  ...
  const jsonLd = {
    ...
    keywords: article.tags.join(", "),
    url: `https://cvixeo.com/nl/careers/${article.slug}`,
  };
        ...
        <div className={`relative overflow-hidden bg-linear-to-br ${style.gradient}`}>
          ...
          <div className="relative mx-auto max-w-3xl px-6 py-16 text-white">
            ...
            <div className="mt-5 flex flex-wrap items-center gap-4 text-xs text-white/60">
              ...
              {article.updatedAt && (
                <span className="flex items-center gap-1.5">
                  Bijgewerkt op {formatDate(article.updatedAt)}
                </span>
              )}
              <span className="flex items-center gap-1.5">
